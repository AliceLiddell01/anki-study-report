...
## G2.4 Learn XP pre-results implementation artifacts
...
The implementation consumes only the frozen `340/340` manifest generated by `learn_candidate_protocol.py`. The publication barrier remains active: no canonical screening run, result evidence, candidate status or family survivor is recorded before the implementation commit is published remotely.
...
The G2.4 commands are pre-results research surfaces. `run-learn-xp-screening` must not be used before the implementation commit is published and remote SHA equality is confirmed.

## Evidence and production boundary

G0.7, G1.2a, G1.4 and G1.5 are synthetic evidence. G2.1 is a prospective contract freeze. G2.2 adds deterministic synthetic lifecycle fixtures and conformance evidence. G2.3 adds prospective candidate protocol and dry matrix identities. G2.4 currently adds pre-results harness/schema/tests, but no screening outcomes or production evidence.
