...
## G2.5 canonical evidence identity
...
`C-CONFIRMATION-ONLY-D1-NOTE-SIBLING` and `C-PENDING-SPLIT-D1-NOTE-SIBLING` are
`CONFIRMATORY_INCONCLUSIVE` with reason `DISPOSABLE_ANKI_IDENTITY_PROBE_UNAVAILABLE`.
`R-NO-LEARN-XP-NOTE-SIBLING` is `REFERENCE_ONLY`.
