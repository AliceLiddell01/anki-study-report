...
The frozen pre-results matrix contains exactly two survivors, one zero-reward reference, 36 conditions and two replay identities for `216` unique units. Identity mode is `SYNTHETIC_CONTRACT_ONLY`; missing runtime-compatible identity evidence is fail-closed as `CONFIRMATORY_INCONCLUSIVE`. No result or outcome is committed in the publication state.
...
The G2.4 commands remain historical research-only surfaces and G2.4 is not rerun in G2.5. The G2.5 run command is prohibited until the frozen protocol and implementation are committed, pushed, the local/remote publication SHA is equal, and focused plus one full research suite pass before result access.

## Evidence and production boundary

G0.7, G1.2a, G1.4 and G1.5 are synthetic evidence. G2.1 is a prospective contract freeze. G2.2 adds deterministic synthetic lifecycle fixtures and conformance evidence. G2.3 adds prospective candidate protocol and dry matrix identities. G2.4 records one invalid disclosed synthetic attempt and one valid full replacement run. G2.5 now contains only prospectively frozen protocol and implementation assets; confirmatory results have not been accessed. The valid external bundles remain owner-managed and are not part of Git; no production evidence or approval exists.
