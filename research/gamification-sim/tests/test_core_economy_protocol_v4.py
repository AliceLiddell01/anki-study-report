...
from gamification_sim.core_economy_protocol_v4 import (
    HISTORICAL_SHA256,
    LEARN_LIMITATION,
...
HUMAN_PATH = REPOSITORY_ROOT / "docs" / "gamification" / "core-economy-candidate-protocol-v4.md"


def test_v4_artifacts_are_strict_schema_valid_and_semantically_valid() -> None:
    artifacts = build_all()
...
def test_every_scenario_carries_required_metrics_for_each_gate() -> None:
    artifacts = build_all()
    gates = {
        item["gate_id"]: item
...
def test_generated_workspace_and_negative_corpus_are_reproducible() -> None:
    assert build_negative_corpus()["identity"]["version"] == 4
    ...
    assert "G4.3 v3: `INVALIDATED_AFTER_SUBSTANTIVE_DEFECT`" in human
    assert "G4.3 v4: `FROZEN_PRE_SCREENING`" in human
