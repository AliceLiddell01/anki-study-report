...
def episode_from_dict(data: dict[str, Any]) -> ReviewEpisodeInput:
    return ReviewEpisodeInput(
        ...
        core_eligibility=data.get("core_eligibility", 1),
...
def day_from_dict(data: dict[str, Any]) -> ReviewDayInput:
    ...
    for group in data.get("repeat_episodes", []):
        ...
        if count < 0:
            raise ValueError("repeat_episodes count must be non-negative")
    ...
    workload = WorkloadSnapshot(
        ...
        natural_due_at_start=workload_data.get("natural_due_at_start", 0),
        due_visible_under_limits=workload_data.get("due_visible_under_limits", 0),
        due_hidden_by_limits=workload_data.get("due_hidden_by_limits", 0),
    ...
    return ReviewDayInput(
        ...
        support_events=tuple(
            SupportEventInput(
                ...
                units=item["units"],
                kind=SupportKind(item.get("kind", "other")),
            )
