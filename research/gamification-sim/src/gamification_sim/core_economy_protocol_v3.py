...
PROTOCOL_VERSION = 3
PIPELINE_VERSION = 3
VALIDATOR_VERSION = "core-economy-protocol-v3-validator-1"
GENERATOR_VERSION = "core-economy-protocol-v3-generator-1"
EPSILON = 1e-12
ROUNDING_DECIMALS = 12

PROTOCOL_PATH = Path("contracts/core-economy-candidate-protocol-v3.json")
PROTOCOL_SCHEMA_PATH = Path("schemas/core-economy-candidate-protocol-v3.schema.json")
PIPELINE_PATH = Path("contracts/core-economy-evaluation-pipeline-v3.json")
PIPELINE_SCHEMA_PATH = Path("schemas/core-economy-evaluation-pipeline-v3.schema.json")
SCENARIOS_PATH = Path("fixtures/core-economy-candidate-scenarios-v3.json")
SCENARIOS_SCHEMA_PATH = Path("schemas/core-economy-candidate-scenarios-v3.schema.json")
MATRIX_PATH = Path("matrices/core-economy-screening-matrix-v3.json")
MATRIX_SCHEMA_PATH = Path("schemas/core-economy-screening-matrix-v3.schema.json")
NEGATIVE_CORPUS_PATH = Path("fixtures/core-economy-candidate-protocol-v3-negative/manifest.json")
HUMAN_PROTOCOL_REPOSITORY_PATH = Path("docs/gamification/core-economy-candidate-protocol-v3.md")
ROADMAP_CLOSEOUT_REPOSITORY_PATH = Path("roadmap/gamification/g4-core-economy-candidate-protocol-v3.md")
...
def build_pipeline() -> dict[str, Any]:
    pipeline = {
        "$schema": "../schemas/core-economy-evaluation-pipeline-v3.schema.json",
        "identity": {
    ...
    codes.update(
        {
            ...
            "RC-EXPECTED-ABRUPT-CUMULATIVE-FAIL",
...
def build_protocol(
    *,
    ...
    metrics = _metrics()
    protocol = {
        "$schema": "../schemas/core-economy-candidate-protocol-v3.schema.json",
        "identity": {
            "artifact_id": "core-economy-candidate-protocol",
            "version": PROTOCOL_VERSION,
            "status": "FROZEN_PRE_SCREENING",
            "stage": "G4.3_V3_PRE_SCREENING_REPUBLICATION",
            "artifact_digest": "PENDING",
        ...
        "supersession": {
            ...
            "reason_codes": [
                "UNCERTAINTY_STATE_MACHINE_UNDERDEFINED",
                "REVIEW_AXIS_LABEL_ONLY",
                "EXPECTED_OUTCOME_NOT_TYPED_IDENTITY_SPECIFIC",
                "CUMULATIVE_FALSE_POSITIVE_METRIC_NOT_EXECUTED",
                "DAILY_BOUNDING_CLAIM_UNTRUE",
                "DUAL_DOMAIN_MARGINALITY_LINKAGE_INCOMPLETE",
                "INVENTORY_SET_COVERAGE_INCOMPLETE",
            ],
        },
        "governance": {
...
def build_scenarios(protocol_digest: str, pipeline_digest: str) -> dict[str, Any]:
    ...
    artifact = {
        "$schema": "../schemas/core-economy-candidate-scenarios-v3.schema.json",
        "identity": {
...
def _expected_gate_outcome(
    ...
    positive_context = sum(float(event["positive_context"]) for event in scenario["ordered_inputs"])
    review_input = sum(float(event["review_units"]) for event in scenario["ordered_inputs"])
    learn_input = sum(float(event["learn_lru"]) for event in scenario["ordered_inputs"])
    combined_saturates = combined and review_input > 0 and learn_input > 0 and review_input + learn_input > 6.0
    outcome = "PASS"
    reason_code = "RC-EXPECTED-PASS"
    predicate_ast = _typed_ast("ALWAYS_PASS")
    if abrupt and positive_context > 0 and gate_id == "HG-FALSE-POSITIVE-CONTEXT-HARM-BOUNDED":
        outcome = "CONTROL_EXPECTED_FAIL"
        reason_code = "RC-EXPECTED-ABRUPT-CONTEXT-FAIL"
        predicate_ast = _typed_ast("AND", candidate="U-ABRUPT-CUTOFF-CONTROL", matched_positive_context_gt=0.0)
    elif abrupt and positive_context > 0 and gate_id == "HG-FALSE-POSITIVE-CUMULATIVE-HARM-BOUNDED":
        outcome = "CONTROL_EXPECTED_FAIL"
        reason_code = "RC-EXPECTED-ABRUPT-CUMULATIVE-FAIL"
        predicate_ast = _typed_ast("AND", candidate="U-ABRUPT-CUTOFF-CONTROL", cumulative_matched_positive_context_gt=0.0)
        ...
        for bundle in _applicable_bundles(protocol, scenario):
            for review_member in members:
                for replay_direction in scenario["replay_directions"]:
                    ...
                    expected = [
                        _expected_gate_outcome(bundle, scenario, gate_id)
...
def build_matrix(protocol: Mapping[str, Any], pipeline: Mapping[str, Any], scenarios: Mapping[str, Any]) -> dict[str, Any]:
    ...
    artifact = {
        "$schema": "../schemas/core-economy-screening-matrix-v3.schema.json",
        "identity": {
...
def _base_schema(artifact_id: str, required: Sequence[str], properties: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "$id": f"https://example.invalid/anki-study-report/{artifact_id}-v3.schema.json",
        "type": "object",
        "additionalProperties": False,
...
def semantic_validate(protocol: Mapping[str, Any], pipeline: Mapping[str, Any], scenarios: Mapping[str, Any], matrix: Mapping[str, Any], *, human_text: str | None = None) -> None:
    if protocol["identity"]["version"] != 3 or pipeline["identity"]["version"] != 3 or scenarios["identity"]["version"] != 3 or matrix["identity"]["version"] != 3:
        _fail("V3_IDENTITY_MIX", "all v3 artifact versions must equal 3")
    ...
    if protocol["supersession"]["prior_versions"] != expected_prior or protocol["supersession"]["results_accessed_before_republication"]:
        _fail("SUPERSESSION_STATE", "v1/v2 must be superseded before execution with no results")
    ...
    for row in matrix["rows"]:
        ...
        expected_outcomes = [
            _expected_gate_outcome(bundles[row["candidate_bundle_id"]], scenario, gate_id)
            for gate_id in scenario["applicable_gate_ids"]
        ]
...
def build_negative_corpus() -> dict[str, Any]:
    samples = [
        {"sample_id": "NEG-VERSION", "target": "protocol", "operation": "set", "path": "/identity/version", "value": 2, "expected_error_codes": ["V3_IDENTITY_MIX", "SCHEMA_VALIDATION"]},
        {"sample_id": "NEG-SUPERSESSION-RESULT-ACCESS", "target": "protocol", "operation": "set", "path": "/supersession/results_accessed_before_republication", "value": True, "expected_error_codes": ["SUPERSESSION_STATE"]},
        ...
        {"sample_id": "NEG-HUMAN-PARITY", "target": "human", "operation": "replace", "path": "G4.3 v3: `FROZEN_PRE_SCREENING`", "value": "G4.3 v3: `BROKEN`", "expected_error_codes": ["HUMAN_MACHINE_PARITY"]},
    ...
    return {
        "identity": {
            "artifact_id": "core-economy-candidate-protocol-v3-negative-corpus",
            "version": 3,
            "generator_version": GENERATOR_VERSION,
        },
        "samples": samples,
...
def validate_workspace(root: Path, *, repository_root: Path | None = None) -> dict[str, Any]:
    ...
    if corpus["identity"]["version"] != 3 or corpus != build_negative_corpus():
        _fail("NEGATIVE_CORPUS_COUNT", "v3 negative corpus must be byte-derived from generator")
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate and validate the G4.3 v3 pre-screening republication")
