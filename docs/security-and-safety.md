...
Cards preview не использует iframe и не исполняет JavaScript templates. Для
`table`/`tiles` CSS карточек изолируется Shadow DOM, а `ankiPreview` показывает
уже sanitized answer-only HTML из `renderedPreview.backHtml`. Эти ограничения
являются частью security contract, а не только визуальной реализации.
