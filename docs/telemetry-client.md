...
Python заново валидирует точную event union, добавляет доверенные
`addonVersion`, `ankiVersion`, coarse OS/locale/theme и локальный `eventId`,
после чего записывает событие в очередь. Version contract fields не дублируются
внутри каждого event: telemetry/consent/privacy versions передаются при
enrollment, а `telemetrySchemaVersion` также находится в batch envelope.
