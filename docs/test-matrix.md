...
| Inspection Profiles / triage v2 | profile store/service/runtime/triage/dashboard pytest + JSON Schema parity + profile/triage Vitest/typecheck | exact-SHA Fast CI, targeted `standard/cards` with `verify_restart=true` | Да | Live note-type fields/templates/fingerprint, profile-local atomic persistence, Japanese/Programming policy separation, confirmed-only reason, template-reference drift to needs_review and restart survival require real Anki |
