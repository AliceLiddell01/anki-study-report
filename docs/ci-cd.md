...
Stage 2 создаёт только producer contract. Full Docker / Anki E2E всё ещё строит
package из source; cross-run download и потребление Fast package относятся к
отдельному следующему этапу.
...
- Codex CI consumer;
- cross-workflow download или E2E consumption exact Fast package;
...
inputs `mode`, `scope`, `screenshot_workers`, `resource_telemetry` и
`verify_restart`:
...
`push` trigger; перед merge он удаляется. Performance-изменения проверяют
