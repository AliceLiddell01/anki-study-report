# G4.3 v3 — исполняемый протокол core economy
...
results accessed before republication: false
G4.4: NOT STARTED
```

v3 является новой versioned republication. Исторические v1/v2 machine artifacts, generator, validator и tests остаются неизменными и проверяются по SHA-256. Этот документ не содержит screening results и не выбирает winner.
...
```text
abrupt + positive context -> CONTROL_EXPECTED_FAIL
abrupt + zero positive context -> PASS
...
```text
research/gamification-sim/contracts/core-economy-candidate-protocol-v3.json
research/gamification-sim/contracts/core-economy-evaluation-pipeline-v3.json
research/gamification-sim/fixtures/core-economy-candidate-scenarios-v3.json
research/gamification-sim/matrices/core-economy-screening-matrix-v3.json
research/gamification-sim/schemas/core-economy-candidate-protocol-v3.schema.json
research/gamification-sim/schemas/core-economy-evaluation-pipeline-v3.schema.json
research/gamification-sim/schemas/core-economy-candidate-scenarios-v3.schema.json
research/gamification-sim/schemas/core-economy-screening-matrix-v3.schema.json
research/gamification-sim/fixtures/core-economy-candidate-protocol-v3-negative/manifest.json
```

Generator:

```text
core-economy-protocol-v3-generator-1
```

Validator:

```text
core-economy-protocol-v3-validator-1
```
...
node scripts/run_python.mjs -m gamification_sim.core_economy_protocol_v3 --research-root research/gamification-sim --repository-root . --regenerate --validate --check-byte-identical
node scripts/run_python.mjs -m pytest research/gamification-sim/tests/test_core_economy_protocol_v3.py -q
```

Validation включает strict duplicate-key-safe parse, Draft 2020-12 schema self-check, artifact validation, semantic validation, negative corpus, byte-identical regeneration, exact inventory coverage, source continuity, historical v1/v2 immutability и no-results enforcement.
