...
| --- | --- | --- | --- | --- | --- | --- |
| Hero/banner/avatar composition | Мгновенно сделать профиль личной поверхностью | `profile.identity`, `profile.studyHistory` | IMPROVE | Project-native gradient banner, deterministic initials, factual dates и settings action | Внешние и demo assets не нужны; identity остаётся главным объектом | Profile tests; 1024–4K light/dark screenshots |
...
| Skills cards | Сделать реальные области обучения заметнее analytics | `profile.decks.overview` | IMPROVE | `Learning areas / Области обучения`: deck name, `totalReviews`, `activeDays`; до четырёх cards и bounded remainder | Deck — реальная область данных, но не skill/level/mastery | Data mapping and order tests |
...
| Status surface | Сконцентрировать локальный system-like акцент | `studyHistory` | ADAPT | Одна выразительная factual surface с шестью lifetime metrics и source captions | Сохраняет направление mock без оценок пользователя | Unavailable metric and visual tests |
...
| Activity heatmap | Быстро показать реальную плотность истории | `profile.activity.days`, `rangeStart`, `rangeEnd` | IMPROVE | Bounded 182-day grid, factual summary и link `#/calendar` | Сохраняет честную all-collection activity без interactive noise | Heatmap/empty/responsive tests |
...
- Learning areas используют текущие реальные deck names и только factual review/
  active-day counts.
- Status не делает judgment о дисциплине, интеллекте, стабильности или mastery.
...
- DOM и visual order: hero → learning areas → Status/Activity → recent history.
- Нет labels/values XP, levels, achievements, mastery, quests или speculative routes.
- `profile.decks.overview` является единственным источником learning areas.
