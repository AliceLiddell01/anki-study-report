...
Для нескольких шаблонов Basic предлагает явный выбор: все шаблоны либо
конкретный набор. В режиме конкретного набора нельзя снять последний
выбранный шаблон: переход ко всем шаблонам выполняется только явным выбором
варианта «все». Это предотвращает неявное превращение конкретного scope в
пустой `templateOrdinals`.
...
После явного неудачного действия Check или Confirm фокус переходит на сводку.
Активация ссылки для Basic-supported ошибки сохраняет Basic и переводит фокус
на точный элемент управления. Только ссылка для Advanced-only ошибки сначала
переключает editor в Advanced, затем переводит фокус на строгий control.

Добавление требования переводит фокус в созданный row. После удаления фокус
переходит в следующий либо предыдущий row, а если требований не осталось — в
кнопку добавления. Очистка обязательного field mapping оставляет в черновике
пустой список, помечает selector как недопустимый и через сводку возвращает
фокус на этот selector.
