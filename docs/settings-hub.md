...
`#/profile` остаётся route, но больше не редактирует dashboard settings.
Страница показывает read-only summary текущей области и кнопку «Открыть
настройки». Это transitional surface, не Profile MVP.
