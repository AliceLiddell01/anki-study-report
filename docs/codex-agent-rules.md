...
Первым действием выполнить repository environment preflight:
...
До `Codex environment preflight: PASS` запрещены:

```text
использование task checkout как authoritative current source
branch creation или branch movement
...
При `BLOCKED` Codex немедленно останавливается и сообщает environment issue,
exit code и bounded summary. Нельзя обходить blocker разовым HTTPS override,
implicit current-directory fallback, `/mnt/*` checkout или stale local `core`.

После PASS прочитать в порядке:
...
Не трогать unrelated dirty changes. Если они появились после setup или в ходе
работы, сохранить их и явно отметить в финальном отчёте. Preflight ничего не
удаляет и не fast-forward local branches.
...
Не смешивать Windows и WSL paths без необходимости. В WSL canonical PowerShell
entrypoints обязаны разрешать native Linux tools; Windows `.exe`, `.cmd`, `.bat`
и `/mnt/<drive>/...` executable paths являются blocker.
...
- Не создавать task branch от local `core`; использовать exact SHA свежего
  `origin/core`, подтверждённый preflight.
...
В WSL этот PowerShell entrypoint должен использовать native Linux `git`, `node`
и `pnpm`. Если resolver находит Windows executable, задача блокируется вместо
разового обхода.
...
Commit messages описывают фактически выполненное изменение естественным техническим
языком, без названия этапа и пересказа prompt:
...
Task branch создаётся от exact SHA `origin/core`, который вывел успешный
preflight. Local `core` может отставать и не является основанием task branch.
