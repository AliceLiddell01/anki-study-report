...
## Язык PR и итоговых материалов

Если пользователь или текущий проектный контекст задаёт русский язык для PR, используй русский во всём человекочитаемом тексте PR, closeout-отчётов, roadmap-статусов и handoff-документов. Не смешивай русские предложения с переводимыми английскими словами ради стилистики.

Сохраняй без перевода только точные технические элементы: идентификаторы, пути, команды, имена полей схемы, названия workflow/job, значения enum, SHA/digest, API и устоявшиеся обозначения `Fast CI`, `E2E`, `GHCR`, `APKG`, `JSON`, `YAML`.

Перед публикацией PR проверь:

1. заголовок и все названия разделов написаны на выбранном языке;
2. статусы и выводы не содержат переводимых английских фраз;
3. точные технические значения не были локализованы или искажены;
4. closeout-отчёт содержит SHA, run ID, artifact ID/digest, выполненные проверки, непроверенное и границы области задачи;
5. merge, release и начало следующего этапа остаются отдельными явно разрешёнными действиями.
