...
## Full Docker E2E падает только в GitHub Actions

Сначала разделить infrastructure failure и project failure. Проверить exact-SHA
run, runner preflight, `docker-system.txt`, Compose config, build/E2E log и
`ci-e2e-summary.json`. Ошибка provisioning, GitHub outage или registry/CDN не
равна regression проекта; rerun допустим только при доказанной transient
причине.

Если container запустился, использовать redacted manifest/reports/screenshots:
`anki-startup-tail`, readiness events, API/browser summaries и failure HTML.
Raw `dashboard-ready.json` из public artifact отсутствует намеренно. Ошибка
artifact redaction является настоящим failure и не должна обходиться upload'ом
raw каталога.

Проверить, что mode/scope соответствуют ожиданию: performance bootstrap push
доказывает targeted `standard/stats` с 3 workers, а final manual runs передают
typed `standard/full`. Strict APKG/Perf100 запускаются только при смысловом
изменении Cards/APKG paths. Perf100 timings диагностические; падение
определяется существующими smoke assertions, а не новым threshold.

## E2E медленный или parallel capture нестабилен

Сначала сверить exact SHA, mode, scope, workers и cache state. Targeted run
нельзя сравнивать с full baseline 183 s как равнозначный. Для full смотреть:
...
Slow screenshot диагностируется по task ID/worker/route/theme; collision или
duplicate path — contract failure. Проверить p95 CPU/memory, headroom и worker
utilization, а не один peak. `gha-enabled` означает подключённый backend, но не
доказанный hit: открыть Buildx record/log и сравнить first/warm build duration.
Если sampler не завершился, исправлять cleanup/stop marker, не отключать
failure artifacts. Не повышать workers выше 4, не параллелить APKG/Cards/
settings persistence/restart и не удалять screenshots ради времени.
