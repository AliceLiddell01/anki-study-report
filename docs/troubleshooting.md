...
Проверить, что mode соответствует ожиданию: bootstrap push всегда доказывает
`strict-apkg`, а final manual runs передают typed `strict-apkg` или `perf100`.
Perf100 timings диагностические; падение определяется существующими smoke
assertions, а не новым threshold.
