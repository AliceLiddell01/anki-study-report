...
базы и лимит 64 MiB. Валидная БД с более новой `user_version` отклоняется
fail-closed без quarantine, downgrade или перезаписи. История ограничена 180
днями и 5000 notifications; evidence — allowlist полей, не более 2048 bytes.
...
сигналы. Повторная оценка того же source revision пропускается и не создаёт
дубликат history.
...
| `card.repeated_again` | ≥3 Again при ≥4 ответах за 7 дней | ≥5 Again | один grouped query по семи учебным дням Anki, максимум 50 cards |
...
Одновременные оценки блокируются. `card.repeated_again` привязывает окно к
`col.sched.day_cutoff`; bounded UTC fallback используется только когда scheduler
недоступен. Логи содержат только detector code, bounded duration bucket, counts
и диагностический code — без evidence и entity IDs.
