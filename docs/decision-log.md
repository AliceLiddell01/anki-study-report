...
Оставить в primary navigation только `Сегодня`, `Календарь`, `Колоды` и
...
Support item не показывается, пока не определён реальный внешний URL.
...
routes. Dropdown становится keyboard/focus contract. Новые Stats, FSRS, Search
и Notifications появляются только с реальными workflows, не placeholders.
