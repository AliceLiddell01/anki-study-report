...
Release bundle является отдельным short-lived artifact и не смешивается с
`ci-fast-*`. Публично допустимы только exact `.ankiaddon`, `SHA256SUMS.txt`,
release notes, provenance attestation и machine-readable manifest без paths и
secrets. `release-artifacts/`, `.playwright-auth/`, browser profiles и
deployment reports остаются ignored/runtime outputs. AnkiWeb credentials
хранятся только в protected GitHub Environment; public logs и reports проходят
тот же запрет на secret-like strings, token URLs и private home paths.
