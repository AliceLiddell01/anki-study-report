...
| 2 | Календарь | `#/calendar` | Календарная активность |
...
`SettingsLayout` задаёт минимальную общую оболочку только для уже существующих
экранов:
...
Диагностика
  Источники данных    → #/integrations
  Логи                → #/logs
```

`#/integrations` остаётся read-only диагностикой локальных источников, а не
платформой внешних интеграций. Shell не добавляет новые settings pages и не
меняет controls существующих страниц.
...
Будущая IA может добавить «Активность», «Статистика», «Поиск» и «Уведомления»,
но только вместе с реальными пользовательскими workflows:

- Календарь становится или дополняется Activity только после Activity Feed;
