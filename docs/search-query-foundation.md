...
Этот слой — read-only основа будущего Search v1. Он уже умеет выполнять
нативный запрос Anki для Cards и Notes и возвращать компактные данные выбранной
Card/Note, но намеренно не добавляет `#/search`, страницу, пункт навигации,
таблицу, inspector UI, selection store или mutation API.
...
- `web-dashboard/src/types/search.ts` и `lib/searchApi.ts` — типы и будущий
  клиент; frontend не получает прямой доступ к collection.
...
catalog пока не требуется ни одному shipping consumer и должен оцениваться
вместе с Search UI, чтобы не создавать второй преждевременный source of truth.
