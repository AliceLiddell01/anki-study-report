...
**Current status:** `G0` is Complete; `G1` is In Progress; `G1.1` and its corrective checkpoint are Complete; `G1.2` is Next / Ready; production integration is not approved
...
**Status:** In Progress — G1.1 and corrective checkpoint Complete; G1.2 Next / Ready
...
- `G1.2 — Root-cause attribution`: Next / Ready
...
G1.1 freezes an internal pre-analysis protocol for G1 diagnostics. Its corrective
checkpoint aligns machine-readable metadata without changing decision semantics.
It does not change the reward formula, run diagnostics or select a candidate.
