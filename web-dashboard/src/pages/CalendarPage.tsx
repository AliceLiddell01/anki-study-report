  ...
  return (
    <aside className="rounded-lg border border-ink-700 bg-ink-900/45 p-4" data-testid="activity-day-detail">
      ...
      <div className="mt-5">
        ...
        {visibleDecks.length ? <div className="mt-2 grid gap-2">{visibleDecks.map((deck) => <div key={deck.id} className="activity-day-deck-row rounded-lg border border-ink-700 bg-ink-850 px-3 py-2.5" data-testid="activity-day-deck-row" title={deck.name}><span className="block break-words text-sm font-medium leading-5 text-report-text">{deck.name}</span><span className="mt-1 block text-xs leading-5 text-report-muted">{formatInteger(deck.reviews)} повторений · {deck.successRate === null ? "Нет данных" : formatPercent(deck.successRate)}</span></div>)}</div> : <p className="mt-2 text-sm text-report-muted">Названия активных колод недоступны.</p>}
...
function DailyEntry({ entry, day }: { entry: ActivityFeedDay; day?: ActivityHubDay }) {
  ...
  return <li className="activity-daily-entry rounded-lg border border-ink-700 bg-ink-900/35 px-3.5 py-3" data-feed-type="daily_summary"><span className="sr-only">Дневная сводка. </span><div><p className="text-[11px] uppercase tracking-wide text-report-muted">{entry.date}</p><h4 className="mt-0.5 font-semibold text-report-text">{formatFullDate(entry.date)}</h4></div><p className="mt-1.5 text-sm leading-5 text-report-muted">{dailySummary(day)}{topDeck ? ` · больше всего в «${topDeck.name}»` : ""}</p>{entry.highlights.length ? <ul className="mt-2 grid gap-1.5">{entry.highlights.map((highlight) => <li key={highlight.id} className="flex items-start gap-2 rounded-md border border-report-blue/25 bg-report-blue/10 px-2.5 py-1.5 text-sm text-report-text"><span className="shrink-0 text-[11px] font-semibold uppercase tracking-wide text-report-blue">{highlightLabel(highlight)}</span><span>{highlightText(highlight)}</span></li>)}</ul> : null}</li>;
}

function WeeklyEntry({ week }: { week: ActivityWeekSummary }) {
  return <li className="rounded-lg border border-report-purple/30 bg-report-purple/10 px-3.5 py-3" data-feed-type="weekly_summary"><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-[11px] uppercase tracking-wide text-report-muted">{week.weekStart} — {week.weekEnd}</p><h4 className="mt-0.5 font-semibold text-report-text">Итоги завершённой недели</h4></div><span className="status-pill status-neutral">неделя</span></div><p className="mt-2 text-sm leading-5 text-report-muted">{formatInteger(week.activeDays)} активных дней · {formatInteger(week.reviews)} повторений · {formatDurationSeconds(week.studySeconds)} · успешность {formatPercent(week.successRate)}</p>{week.comparison ? <p className="mt-2 text-sm font-medium text-report-text">{weeklyComparisonText(week)}</p> : null}</li>;
}
...
function activityCellValue(day: ActivityHubDay, metric: ActivityMetric) { if (day.availability === "unavailable") return "н/д"; if (day.availability === "inactive") return "—"; const value = activityMetricValue(day, metric); if (value === null) return "н/д"; if (metric === "success_rate") return `${Math.round(value * 100)}%`; if (metric === "study_time") return value < 60 ? "<1м" : `${Math.round(value / 60)}м`; return formatInteger(value); }
...
function highlightLabel(highlight: ActivityHighlight) { if (highlight.type === "return_after_break") return "возвращение"; if (highlight.type === "streak_milestone") return "серия"; return "рекорд"; }
