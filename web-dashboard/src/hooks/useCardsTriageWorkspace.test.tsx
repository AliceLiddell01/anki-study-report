...
describe("useCardsTriageWorkspace", () => {
  it("queries automatic v2 once and inspects only the active item", async () => {
    ...
    expect(calls.filter((call) => call.url.includes("/api/triage/query"))).toHaveLength(1);
    const request = calls.find((call) => call.url.includes("/api/triage/query"))!.body as { dataset: string; limit: number; scope: { deckIds: string[]; periodStartMs: number; periodEndMs: number } };
    expect(request.dataset).toBe("automatic"); expect(request.limit).toBe(100); expect(request.scope.deckIds).toEqual(["3"]); expect(request.scope.periodEndMs - request.scope.periodStartMs).toBe(7 * 24 * 60 * 60 * 1000);
...
function Harness() { const workspace = useCardsTriageWorkspace(["3"]); return <div>{workspace.response?.items.map((value) => <button key={value.cardId} data-card-id={value.cardId} onClick={() => workspace.activate(value)}>{value.primaryText}</button>)}</div>; }
function item(cardId: string, primaryText: string) { const reason = { reasonId: `learning:${cardId}`, code: "learning.leech", family: "learning", scope: "card", priority: "high", sources: ["attention"], evidence: [{ kind: "leech_state", lapses: 2 }], detectedAtMs: 2 }; return { itemId: `card:${cardId}`, availability: "available", cardId, noteId: `2${cardId}`, deck: { deckId: "3", name: "Deck" }, noteType: { noteTypeId: "7", name: "Basic" }, template: { ordinal: 0, name: "Card 1" }, primaryText, priority: "high", primaryReasonCode: "learning.leech", reasons: [reason], sources: ["attention"], cardState: { state: "review", suspended: false, buried: false, flag: 0 }, inspect: { mode: "cards", cardId } }; }
