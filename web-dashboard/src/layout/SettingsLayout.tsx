import { Database, FileText, Plug, Server } from "lucide-react";
...
const settingsSections: Array<{
  ...
  {
    label: "Данные",
    items: [{ path: "/settings", label: "Данные", icon: Database }],
  },
  {
    label: "Система",
  ...
  },
  {
    label: "Диагностика",
    items: [
      { path: "/integrations", label: "Источники данных", icon: Plug },
    ...
    <div className="grid gap-5 lg:grid-cols-[240px_minmax(0,1fr)] lg:items-start">
      <aside className="settings-nav-shell rounded-xl border border-ink-700 bg-ink-850 p-3 shadow-panel lg:sticky lg:top-[88px]">
        <div className="px-2 pb-3 pt-1">
          ...
          <p className="mt-1 text-xs leading-5 text-report-muted">Данные, система и диагностика</p>
        </div>
        <nav className="grid gap-3" aria-label="Разделы настроек">
                ...
                {section.items.map((item) => {
                  ...
                  return (
                    <a
                      ...
                      href={`#${item.path}`}
                      className={[
                        "flex min-h-10 items-center gap-2.5 rounded-lg border px-2.5 py-2 text-sm font-medium transition",
                        active
